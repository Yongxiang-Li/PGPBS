function [ fit ] = fit_PGPBS( X, Y, regr, corr, bs, lob, upb)
% Periodic Gaussian Process Modeling Using Bspline
    ...
    % explore
    while length(T) < sum(X>=lob(1)&X<=upb(1))*6
        [~, minIndex] = min(objs);
        maxT = T(minIndex);
        candT = (T(1:end-1)+T(2:end))/2;
        acceptProb = (T(2:end)-T(1:end-1)).*exp(-(objs(1:end-1)+objs(2:end))/2/length(Y));
        rndObj = rand(size(acceptProb))*(max(acceptProb)-min(acceptProb)) + min(acceptProb);
        index = acceptProb>=rndObj;    candT(~index) = nan;
        candObjk = nan(length(candT),r);
        for k = 1 : r
            data.bs = bsplines(k);
            for i = 1 : length(candT)
                if index(i)
                    thetai = [repmat(candT(i),size(paras,1),1) paras];
                    obji = obj_func(thetai', data);
                    candObjk(i,k) = mean(obji);
                    if minObj(k)>min(obji), minObj(k)=min(obji); end
                end
            end
        end
        newT = nan(length(T)*2-1,1);    newT(1:2:end) = T;   newT(2:2:end) = candT;
        newObjk = nan(length(T)*2-1,r);    newObjk(1:2:end,:) = objk;    newObjk(2:2:end,:) = candObjk;
        index = ~isnan(newT);    T = newT(index);    objk = newObjk(index,:);    
        for p = 2:5
            if maxT/p < lob(1), continue; end
            index = find(T>maxT/p, 1, 'first');
            T = [T(1:index-1); maxT/p; T(index:end)];
            addObjk = nan(1,r);
            for k = 1 : r
                data.bs = bsplines(k);
                thetai = [repmat(maxT/p,size(paras,1),1) paras];
                obji = obj_func(thetai', data);
                addObjk(k) = mean(obji);
                if minObj(k)>min(obji), minObj(k)=min(obji); end
            end
            objk = [objk(1:index-1,:); addObjk; objk(index:end,:)];
        end
        objs = mean(objk,2); % figure; plot(T, -objs)
    end
    ...
    % exploit
    t = now;
    if minIndex>1 && minIndex < length(objk)
        lob(1) = T(minIndex-1,1);    upb(1) = T(minIndex+1,1);
    end
    if minIndex==1, upb(1) = T(minIndex+1,1); end
    if minIndex == length(objs), lob(1) = T(minIndex-1,1); end
    theta0 = (lob+upb)/2;
